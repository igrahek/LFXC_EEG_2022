...
for f = 1:length(data)/10 %number of plots
    ...
    for s = f*10-9:f*10 % for every subject
        ...
        hold on
        plot(1:288,predicted(s).runavg,'-','LineWidth',1.5,'DisplayName','Running Average')
        ...
        saveas(h,[results_path,sprintf('%s',type),'\predicted_observed_',sprintf('%s',type),sprintf('%d',f),sprintf('%s',model_fitted),'.pdf'])
