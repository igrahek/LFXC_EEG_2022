...
%% Load in data files
files = dir(sprintf('../../Data/Behavior/raw/%d/ST_*mat',version_exp));
fileNames = {files(:).name};

for curSub = 1:length(fileNames)
    file = [sprintf('../../Data/Behavior/raw/%d/',version_exp),fileNames{curSub}];
...
allSubHdr = {'SubID','Vers','Phase','Block','Trial','EffLvl','RewLvl','Acc','TargetResp','DistractorResp','RT','AccRT','Resp','IsRewarded','Congruency','SliderEnd',...
    'ProbeRt','ProbeType','feedbackOrder'}; %,'ChosenEff','ModeChosenEff','AltEff','ChosenTrialAmt','AltTrialAmt','ChoiceRT','ChoseRight','LeftEffOption',...
