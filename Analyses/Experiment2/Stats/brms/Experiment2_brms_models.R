...
sub.exclude.accuracy = summary_table$SubID[which(summary_table$MeanAcc<thresh_meanAcc_subj)]
...
#Plot the model
pdf(file="C:/Users/igrahek/Dropbox (Brown)/CLPS-ShenhavLab/EEG_Studies/Experiments/LFXC_EEG/Manuscript/plots/Figures/Exp2/Figure_Clustering_A.pdf")
...
pdf(file="C:/Users/igrahek/Dropbox (Brown)/CLPS-ShenhavLab/EEG_Studies/Experiments/LFXC_EEG/Manuscript/plots/Figures/Exp2/Figure_Clustering_B1.pdf")
...
pdf(file="C:/Users/igrahek/Dropbox (Brown)/CLPS-ShenhavLab/EEG_Studies/Experiments/LFXC_EEG/Manuscript/plots/Figures/Exp2/Figure_Clustering_B2.pdf")
...
# Save the excluded ones for the plot below
plot_data <- df.interval %>%
...
# Plot the excluded subjects

pdf(file="C:/Users/igrahek/Dropbox (Brown)/CLPS-ShenhavLab/EEG_Studies/Experiments/LFXC_EEG/Manuscript/plots/Figures/ExcludedStudy2.pdf")

  # Efficacy
  
  efficacy_plot = ggplot(data = plot_data, aes(x = plot_data$Interval))+
    geom_point(mapping=aes(y=plot_data$EfficacyProbeResp,color='Actual estimates'),size = 1)+
    geom_line(mapping=aes(y=plot_data$mbased_efficacy,color='Model-based'),size=0.8) +
    
    facet_wrap( ~ SubID,nrow = 6) + 
    # ggtitle(paste0('Efficacy estimates - Subject ',unique(df.interval$SubID)[subject])) +
    scale_color_manual(values = c(
      'Model-based' = 'black',
      'Raw subjective estimates' = 'grey')) +
    labs(color = 'Estimates')+
    theme_classic() +
    # theme(plot.title = element_text(size = 12, face = "bold"))+
    xlab("Interval number") +
    ylab("Subjective efficacy estimate") +
    ylim(0,1.1) 
...
# Contrast code the efficacy levels
df.interval.combined$EffLvl = ifelse(df.interval.combined$EfficacyLevel == 0, "NoEfficacy", "Efficacy")
df.interval.combined$EffLvl = as.factor(df.interval.combined$EffLvl)
...
data_hddm = data_hddm %>% dplyr::select(subj_idx,response, rt, Congruency,mbased_reward_prev,mbased_efficacy_prev)
...
write.csv(data_hddm,"../ddm/data/LFXC/ae_only/data_hddm_LFXC_interval_allTrials.csv", row.names=FALSE)

# Save out without subject 51
data_hddm = subset(data_hddm,data_hddm$subj_idx!=51)
write.csv(data_hddm,"../ddm/data/LFXC/ae_only/data_hddm_LFXC_interval_allTrials_no51.csv", row.names=FALSE)
...
rcorr(learning_rates_efficacy$positive_learning_rate,learning_rates_efficacy$BIC,type = "spearman")

PositiveLR<-ggplot(learning_rates_efficacy,aes(x= positive_learning_rate, y=BIC)) +
  geom_point(size=3) +
...
rcorr(learning_rates_efficacy$negative_learning_rate,learning_rates_efficacy$BIC,type = "spearman")

NegativeLR<-ggplot(learning_rates_efficacy,aes(x= negative_learning_rate, y=BIC)) +
  geom_point(size=3) +
  ggtitle('Efficacy learning rates and model fit')+
  ylab("BIC") +
...
# Positive learning rates
rcorr(learning_rates_reward$positive_learning_rate,learning_rates_reward$BIC,type = "spearman")

PositiveLR<-ggplot(learning_rates_reward,aes(x= positive_learning_rate, y=BIC)) +
  geom_point(size=3) +
...
# Negative learning rates
rcorr(learning_rates_reward$negative_learning_rate,learning_rates_reward$BIC,type = "spearman")

NegativeLR<-ggplot(learning_rates_reward,aes(x= negative_learning_rate, y=BIC)) +
  geom_point(size=3) +
  ggtitle('Reward learning rates and model fit')+
  ylab("BIC") +
  xlab("Negative learning rate") +
...
data.efficacy = df.trial.combined

data.efficacy = data.efficacy[, c('SubID', 'AccRT', 'hit',"mbased_efficacy_prev")]
...
df.quartiles = cbind(data.efficacy$SubID,data.efficacy$AccRT,data.efficacy$hit,data.efficacy$quartile)
colnames(df.quartiles) = c("SubID","AccRT","hit","quartile")
...
pdf(file="C:/Users/igrahek/Dropbox (Brown)/CLPS-ShenhavLab/EEG_Studies/Experiments/LFXC_EEG/Manuscript/plots/Figures/FigureS8_C.pdf") 
...
summary <- Rmisc::summarySEwithin(df.quartiles, measurevar="hit",
                                  withinvars=c("quartile"),
                                  idvar="SubID", na.rm=TRUE, conf.interval=.95)

summary = summary[-6,]

pdf(file="C:/Users/igrahek/Dropbox (Brown)/CLPS-ShenhavLab/EEG_Studies/Experiments/LFXC_EEG/Manuscript/plots/Figures/FigureS8_D.pdf") 

Acc_Plot<-ggplot(summary,aes(x= quartile, y=hit)) +
  geom_line(size=1,position = position_jitter(width=0.1,seed = 123))+
  geom_point(size=3,position = position_jitter(width=0.1,seed = 123)) +
  geom_linerange(size=1, aes(ymin=hit-se,ymax=hit+se),position = position_jitter(width=0.1,seed = 123)) + 
  ggtitle('Accuracy')+
  ylab("Accuracy") +
  xlab("Model-based efficacy quartile") +
  scale_y_continuous(breaks=seq(0.85, 0.90, 0.025), limits=c(0.85, 0.90))+
  theme(axis.line = element_line(colour = "black"),plot.margin=grid::unit(c(5,15,5,5), "mm"),
        axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0)),
        text = element_text(size=20),
        panel.grid.major = element_blank(),
...
#### Save out for brms ####
write.csv(df.trial.combined,"Cluster/data/Exp2Data.csv", row.names=FALSE)
write.csv(df.interval.combined,"Cluster/data/Exp2DataInterval.csv", row.names=FALSE)

# remove the LRs of the excluded participants

learning_rates_efficacy_save <- learning_rates_efficacy %>%
  filter(SubID %in% unique(df.trial.combined$SubID)) 

write.csv(learning_rates_efficacy_save,"Cluster/data/Exp2LRsEfficacy.csv", row.names=FALSE)

learning_rates_reward_save <- learning_rates_reward %>%
  filter(SubID %in% unique(df.trial.combined$SubID)) 

write.csv(learning_rates_reward_save,"Cluster/data/Exp2LRsReward.csv", row.names=FALSE)
...
model.reward_probability_EffLvl = brm(reward ~ EffLvl + (EffLvl|SubID),
                                         data=df.interval.combined,
                                         family=gaussian(),
                                         iter  = 10000,
                                         warmup = 8000,
                                         ...
                                         sample_prior = TRUE)
saveRDS(model.reward_probability_EffLvl,file="model.model.reward_probability_EffLvl.rds")

# Model efficacy model-based
model.reward_probability_Effmbased = brm(reward ~ mbased_efficacy_prev + (mbased_efficacy_prev|SubID),
                                         data=df.interval.combined,
                                         family=gaussian(),
                                         iter  = 10000,
                                         warmup = 8000,
                                         ...
                                         control = list(adapt_delta = 0.99),
                                         sample_prior = TRUE)
saveRDS(model.reward_probability_Effmbased,file="model.model.reward_probability_Effmbased.rds")
...
model.Rewmbased_Effmbased = brm(mbased_reward ~ mbased_efficacy + (mbased_efficacy|SubID),
                                ...
                                control = list(adapt_delta = 0.99),
                                sample_prior = TRUE)
saveRDS(model.Rewmbased_Effmbased,file="model.model.Rewmbased_Effmbased.rds")
...
reward = brm(RewardProbeResp ~  mbased_reward + mbased_efficacy + 
...
# # Set the working directory where to save the models
# setwd(here("Analyses/Experiment2/Stats/brms/brms_models"))
# 
