% Model with the intercept fixed at 0.5
...
if ~any(strcmp(char({files.name}),version))
        ...
        opts = detectImportOptions(fullfile(root, version, ['RL_fit_data_',version,'.csv']));
        opts.VariableTypes = {'double','double','double','double','double','double','double','double','double','double','double','double','double'};
        allSubData = readtable(fullfile(root, version, ['RL_fit_data_',version,'.csv']),opts);
...
% add the fixed initial value to the results
init_v = repmat(0.5,length(subs),1);
init_v = init_v';
results.x(:,1) = init_v; 
...
for f = 1:length(data)/10 %number of plots
    ...
    for s = f*10-9:f*10 % for every subject
        ...
        title(['sub=',sprintf('%1.0f',results.x(s,2)),'; fixed intercept=', sprintf('%1.3f',results.x(s,1)), '; R2= ', sprintf('%1.3f',r2.R2(s)),'; AIC= ', sprintf('%1.3f',AIC(s)),'; BIC= ', sprintf('%1.3f',BIC(s))])
        ylim([-0.1 1.1])
    end
...
%     for s = f*10-9:f*10 % for every subject
%         subplot(5,2,s-((f-1)*10))
%         plot(1:288,predicted(s).actual_value,'o','DisplayName','Subjective estimate', 'MarkerFaceColor', 'b','MarkerSize',4)
%         legend ('Location','NorthEastOutside')
%         hold on
%         plot(1:288,predicted(s).value,'-','LineWidth',1.5,'DisplayName','Predicted values')
%         xlabel('Trial','FontSize',10);
%         ylabel('Estimate','FontSize',10);
%         hold on
%         plot(1:288,predicted(s).feedback,'o','DisplayName','Feedback', 'MarkerFaceColor', 'k','MarkerEdgeColor','k','MarkerSize',2)
%         %title(['sub=',sprintf('%1.0f',s),'; lr=', sprintf('%1.3f',results.x(s,1)),'; vinit= ', sprintf('%1.3f',results.x(s,2))])
%         title(['sub=',sprintf('%1.0f',results.x(s,2)),'; fixed intercept=', sprintf('%1.3f',results.x(s,1)), '; R2= ', sprintf('%1.3f',r2.R2(s)),'; AIC= ', sprintf('%1.3f',AIC(s)),'; BIC= ', sprintf('%1.3f',BIC(s))])
%         ylim([-0.1 1.1])
%     end
