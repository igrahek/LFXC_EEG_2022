function results = mfit_optimize_hierarchical(likfun,param,data,type,nstarts,parallel)
    ...
    while iter < maxiter
        ...
        if parallel
            results = mfit_optimize_parallel(likfun,param,data,type,nstarts);
        else
            results = mfit_optimize(likfun,param,data,type,nstarts);
        end
        ...
        lme(iter) = sum(L) - K*log(sum([data.N]));
                disp(['Marginal likelihood ',num2str(lme(iter))]);
