function results = mfit_optimize_parallel(likfun,param,data,type,nstarts)
    ...
    if nargin < 5 || isempty(nstarts); nstarts = 5; end
    K = length(param);
    S = length(data);
    results.K = K;
    results.S = S;
    ...
    parfor s = 1:S
        ...
        f = @(x) -mfit_post(x,param,data(s),likfun,type);
