...
param(2).name = 'intercept';
param(2).logpdf = @(x) sum(log(normpdf(x,mu,sigma)));
param(2).lb = 0;
param(2).ub = 1;

param(3).name = 'variance';
param(3).logpdf = @(x) 0; % uniform prior
param(3).lb = 0;
param(3).ub = 0.5;
...
for s = 1:length(data) % for every subject
    for n = 1:data(s).N % for every trial
        ...
        if strcmp(type,'reward')==1
            ...
            v = v + results.x(s,1)* rpe; % update value
...
for f = 1:length(data)/10 %number of plots
    ...
    for s = f*10-9:f*10 % for every subject
        ...
        title(['sub=',sprintf('%1.0f',results.x(s,3)),'; lr=', sprintf('%1.3f',results.x(s,1)),'; vinit= ', sprintf('%1.3f',results.x(s,2)), '; R2= ', sprintf('%1.3f',r2.R2(s)),'; AIC= ', sprintf('%1.3f',AIC(s)),'; BIC= ', sprintf('%1.3f',BIC(s))])
        ylim([-0.1 1.1])
    end
